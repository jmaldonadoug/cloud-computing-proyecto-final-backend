...
const store = async (req, res) => {
    ...
    try {
        ...
        const query = 'INSERT INTO editorial(nombre) VALUES(?);';
...
const edit = async (req, res) => {
    ...
    try {
        const { id } = req.params;
        const query = 'SELECT * FROM editorial WHERE ideditorial = ?;';
        const editorial = await db.awaitQuery(query, [id]);

        response.data = editorial[0];
    } catch (e) {
        response = helpers.failRequest(e, req, res);
    }
...
const update = async (req, res) => {
    ...
    try {
        const { id } = req.params;
        const { nombre } = req.body;
        const query = 'UPDATE editorial SET nombre = ? WHERE ideditorial = ?;';
...
const destroy = async (req, res) => {
    ...
    try {
        const { id } = req.params;
        const query = 'DELETE FROM editorial WHERE ideditorial = ?;';
